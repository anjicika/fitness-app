...
router.post('/advice', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    const userProfile = {
      ...
      dietaryRestrictions: req.body.dietaryRestrictions || req.user.dietaryRestrictions
    };
...
router.post('/meal-plan', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const mealPlan = await aiNutritionService.generateMealPlan(userProfile, days);
...
router.get('/calories', authMiddleware, async (req, res) => {
  try {
    ...
    const macroSplit = aiNutritionService.getMacroSplit(userProfile.fitnessGoal);
    const macroGrams = aiNutritionService.calculateMacroGrams(dailyCalories, macroSplit);

    let bmr;
    if (userProfile.gender === 'male') {
      bmr = Math.round((10 * userProfile.weight) + (6.25 * userProfile.height) - (5 * userProfile.age) + 5);
    } else {
      bmr = Math.round((10 * userProfile.weight) + (6.25 * userProfile.height) - (5 * userProfile.age) - 161);
    }
 ...
 * @limit   10 requests per 15 minutes
 */
router.post('/analyze-meal', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        recommendations: {
          weight_loss: 'Higher protein to preserve muscle, moderate carbs and fats',
